...
import { useGetAllZhovtiVodyNewsQuery, useRemoveZhovtiVodyNewsMutation } from '@/redux/services/regionsNews/Dnipro/dnipro';

const ZhovtiVodyPostCard: FC<News> = ({
  id,
  publishedAt,
  feed,
  section,
  ...
  const [removeZhovtiVodyNews] = useRemoveZhovtiVodyNewsMutation();
  const [isDeleteModalOpen, setIsDeleteOpenModal] = useState(false);
  const { refetch } = useGetAllZhovtiVodyNewsQuery();
  ...
  const handleDeletePost = async () => {
    ...
    if (
      ...
      try {
        await removeZhovtiVodyNews(id).unwrap();
        success();
        refetch();
      } catch (err) {
        ...
        <div className="flex items-center justify-between">
          <div className="flex w-full items-center justify-between">
            <div className="w-full max-w-[600px] text-cyan-700">
              <Link to={`${Paths.zhovtiVodyNews}/${id}`}>{title}</Link>
            </div>
          ...
          <div className="flex gap-1">
            ...
            (currentUserRole === 'Менеджер Західного МР' &&
              ...
              <Button
                type={'text'}
                onClick={() => {
                  navigate(`${Paths.editZhovtiVodyNews}/${id}`);
                }}
                icon={<EditOutlined />}
              ></Button>
            ) : null}
...
export default ZhovtiVodyPostCard;
