...
const PereyaslavPostCard: FC<News> = ({
  id,
  publishedAt,
  feed,
  section,
  ...
  const [removePereyaslavNews] = useRemovePereyaslavNewsMutation();
  const [isDeleteModalOpen, setIsDeleteOpenModal] = useState(false);
  const { refetch } = useGetAllPereyaslavNewsQuery();
  ...
  const handleDeletePost = async () => {
    ...
    if (
      ...
      try {
        await removePereyaslavNews(id).unwrap();
        success();
        refetch();
      } catch (err) {
        ...
        <div className="flex items-center justify-between">
          <div className="flex w-full items-center justify-between">
            <div className="w-full max-w-[600px] text-cyan-700">
              <Link to={`${Paths.pereyaslavNews}/${id}`}>{title}</Link>
            </div>
          ...
          <div className="flex gap-1">
            ...
            (currentUserRole === 'Менеджер Західного МР' &&
              ...
              <Button
                type={'text'}
                onClick={() => {
                  navigate(`${Paths.editPereyaslavNews}/${id}`);
                }}
                icon={<EditOutlined />}
              ></Button>
            ) : null}
...
export default PereyaslavPostCard;
