...
const SkvyraPostCard: FC<News> = ({
  ...
  const [removeSkvyraNews] = useRemoveSkvyraNewsMutation();
  const [isDeleteModalOpen, setIsDeleteOpenModal] = useState(false);
  const { refetch } = useGetAllSkvyraNewsQuery();
  ...
  const handleDeletePost = async () => {
    ...
    if (
      ...
      try {
        await removeSkvyraNews(id).unwrap();
        success();
        refetch();
      } catch (err) {
        ...
        <div className="flex items-center justify-between">
          <div className="flex w-full items-center justify-between">
            <div className="w-full max-w-[600px] text-cyan-700">
              <Link to={`${Paths.skvyraNews}/${id}`}>{title}</Link>
            </div>
          ...
          <div className="flex gap-1">
            ...
            (currentUserRole === 'Менеджер Західного МР' &&
              ...
              <Button
                type={'text'}
                onClick={() => {
                  navigate(`${Paths.editSkvyraNews}/${id}`);
                }}
                icon={<EditOutlined />}
              ></Button>
            ) : null}
