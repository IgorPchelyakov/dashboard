...
const IzmailPostCard: FC<News> = ({
  ...
  const [removeIzmailNews] = useRemoveIzmailNewsMutation();
  const [isDeleteModalOpen, setIsDeleteOpenModal] = useState(false);
  const { refetch } = useGetAllIzmailNewsQuery();
  ...
  const handleDeletePost = async () => {
    ...
    if (
      ...
      try {
        await removeIzmailNews(id).unwrap();
        success();
        refetch();
      } catch (err) {
        ...
        <div className="flex items-center justify-between">
          <div className="flex w-full items-center justify-between">
            <div className="w-full max-w-[600px] text-cyan-700">
              <Link to={`${Paths.izmailNews}/${id}`}>{title}</Link>
            </div>
          ...
          <div className="flex gap-1">
            ...
            (currentUserRole === 'Менеджер Західного МР' &&
              ...
              <Button
                type={'text'}
                onClick={() => {
                  navigate(`${Paths.editIzmailNews}/${id}`);
                }}
                icon={<EditOutlined />}
              ></Button>
            ) : null}
