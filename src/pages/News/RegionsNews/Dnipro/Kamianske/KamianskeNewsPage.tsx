...
import { useGetKamianskeNewsByIdQuery } from '@/redux/services/regionsNews/Dnipro/dnipro';
moment.locale('uk');

const KamianskeNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetKamianskeNewsByIdQuery(params.id || '');
...
export default KamianskeNewsPage;
