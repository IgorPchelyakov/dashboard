...
import { useGetKryvyiRihNewsByIdQuery } from '@/redux/services/regionsNews/Dnipro/dnipro';
moment.locale('uk');

const KryvyiRihNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetKryvyiRihNewsByIdQuery(params.id || '');
...
export default KryvyiRihNewsPage;
