...
import { useGetMarganetsNewsByIdQuery } from '@/redux/services/regionsNews/Dnipro/dnipro';
moment.locale('uk');

const MarganetsNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetMarganetsNewsByIdQuery(params.id || '');
...
export default MarganetsNewsPage;
