...
import { useGetNikopolNewsByIdQuery } from '@/redux/services/regionsNews/Dnipro/dnipro';
moment.locale('uk');

const NikopolNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetNikopolNewsByIdQuery(params.id || '');
...
export default NikopolNewsPage;
