...
import { useGetNovomoskovskNewsByIdQuery } from '@/redux/services/regionsNews/Dnipro/dnipro';
moment.locale('uk');

const NovomoskovskNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetNovomoskovskNewsByIdQuery(params.id || '');
...
export default NovomoskovskNewsPage;
