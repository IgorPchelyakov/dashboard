...
import { useGetPavlogradNewsByIdQuery } from '@/redux/services/regionsNews/Dnipro/dnipro';
moment.locale('uk');

const PavlogradNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetPavlogradNewsByIdQuery(params.id || '');
...
export default PavlogradNewsPage;
