...
import { useGetPershotravenskNewsByIdQuery } from '@/redux/services/regionsNews/Dnipro/dnipro';
moment.locale('uk');

const PershotravenskNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetPershotravenskNewsByIdQuery(
    params.id || '',
  );
...
export default PershotravenskNewsPage;
