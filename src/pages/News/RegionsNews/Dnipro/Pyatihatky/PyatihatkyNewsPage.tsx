...
import { useGetPyatihatkyNewsByIdQuery } from '@/redux/services/regionsNews/Dnipro/dnipro';
moment.locale('uk');

const PyatihatkyNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetPyatihatkyNewsByIdQuery(params.id || '');
...
export default PyatihatkyNewsPage;
