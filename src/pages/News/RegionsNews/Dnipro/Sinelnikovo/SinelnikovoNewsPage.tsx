...
import { useGetSinelnikovoNewsByIdQuery } from '@/redux/services/regionsNews/Dnipro/dnipro';
moment.locale('uk');

const SinelnikovoNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetSinelnikovoNewsByIdQuery(params.id || '');
...
export default SinelnikovoNewsPage;
