...
import { useGetTernivkaNewsByIdQuery } from '@/redux/services/regionsNews/Dnipro/dnipro';
moment.locale('uk');

const TernivkaNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetTernivkaNewsByIdQuery(params.id || '');
...
export default TernivkaNewsPage;
