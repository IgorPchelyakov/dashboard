...
import { useGetVilnohorskNewsByIdQuery } from '@/redux/services/regionsNews/Dnipro/dnipro';
moment.locale('uk');

const VilnohorskNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetVilnohorskNewsByIdQuery(params.id || '');
...
export default VilnohorskNewsPage;
