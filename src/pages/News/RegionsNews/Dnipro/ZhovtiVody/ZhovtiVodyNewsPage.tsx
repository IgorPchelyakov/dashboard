...
import { useGetZhovtiVodyNewsByIdQuery } from '@/redux/services/regionsNews/Dnipro/dnipro';
moment.locale('uk');

const ZhovtiVodyNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetZhovtiVodyNewsByIdQuery(params.id || '');
...
export default ZhovtiVodyNewsPage;
