...
import { useGetBilacerkvaNewsByIdQuery } from '@/redux/services/regionsNews/Kyiv/bilacerkva';
moment.locale('uk');

const BilaCerkvaNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetBilacerkvaNewsByIdQuery(params.id || '');
...
export default BilaCerkvaNewsPage;
