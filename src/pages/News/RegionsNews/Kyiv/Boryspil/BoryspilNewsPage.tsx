...
import { useGetBoryspilNewsByIdQuery } from '@/redux/services/regionsNews/Kyiv/boryspil';
moment.locale('uk');

const BoryspilNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetBoryspilNewsByIdQuery(params.id || '');
...
export default BoryspilNewsPage;
