...
import { useGetBrovaryNewsByIdQuery } from '@/redux/services/regionsNews/Kyiv/brovary';
moment.locale('uk');

const BrovaryNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetBrovaryNewsByIdQuery(params.id || '');
...
export default BrovaryNewsPage;
