...
import { useGetBuchaNewsByIdQuery } from '@/redux/services/regionsNews/Kyiv/bucha';
moment.locale('uk');

const BuchaNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetBuchaNewsByIdQuery(params.id || '');
