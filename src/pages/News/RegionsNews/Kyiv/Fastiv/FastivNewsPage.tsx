...
import { useGetFastivNewsByIdQuery } from '@/redux/services/regionsNews/Kyiv/fastiv';
moment.locale('uk');

const FastivNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetFastivNewsByIdQuery(params.id || '');
