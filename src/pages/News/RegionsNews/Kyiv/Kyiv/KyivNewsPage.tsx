...
import { useGetKyivNewsByIdQuery } from '@/redux/services/regionsNews/Kyiv/kyiv';
moment.locale('uk');

const KyivNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetKyivNewsByIdQuery(params.id || '');
