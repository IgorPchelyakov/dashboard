...
import { useGetObukhivNewsByIdQuery } from '@/redux/services/regionsNews/Kyiv/obukhiv';
moment.locale('uk');

const ObukhivNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetObukhivNewsByIdQuery(params.id || '');
...
export default ObukhivNewsPage;
