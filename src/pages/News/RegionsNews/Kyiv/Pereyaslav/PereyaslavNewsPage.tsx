...
import { useGetPereyaslavNewsByIdQuery } from '@/redux/services/regionsNews/Kyiv/pereyaslav';
moment.locale('uk');

const PereyaslavNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetPereyaslavNewsByIdQuery(params.id || '');
...
export default PereyaslavNewsPage;
