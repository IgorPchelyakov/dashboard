...
import { useGetSkvyraNewsByIdQuery } from '@/redux/services/regionsNews/Kyiv/skvyra';
moment.locale('uk');

const SkvyraNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetSkvyraNewsByIdQuery(params.id || '');
