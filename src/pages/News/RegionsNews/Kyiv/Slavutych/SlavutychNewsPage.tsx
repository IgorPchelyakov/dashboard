...
import { useGetSlavutychNewsByIdQuery } from '@/redux/services/regionsNews/Kyiv/slavutych';
moment.locale('uk');

const SlavutychNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetSlavutychNewsByIdQuery(params.id || '');
...
export default SlavutychNewsPage;
