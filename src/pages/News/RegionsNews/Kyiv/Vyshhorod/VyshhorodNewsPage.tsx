...
import { useGetVyshhorodNewsByIdQuery } from '@/redux/services/regionsNews/Kyiv/vyshhorod';
moment.locale('uk');

const VyshhorodNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetVyshhorodNewsByIdQuery(params.id || '');
...
export default VyshhorodNewsPage;
