...
import { useGetVyshneveNewsByIdQuery } from '@/redux/services/regionsNews/Kyiv/vyshneve';
moment.locale('uk');

const VyshneveNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetVyshneveNewsByIdQuery(params.id || '');
...
export default VyshneveNewsPage;
