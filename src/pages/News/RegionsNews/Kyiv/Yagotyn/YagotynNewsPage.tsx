...
import { useGetYagotynNewsByIdQuery } from '@/redux/services/regionsNews/Kyiv/yagotyn';
moment.locale('uk');

const YagotynNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetYagotynNewsByIdQuery(params.id || '');
...
export default YagotynNewsPage;
