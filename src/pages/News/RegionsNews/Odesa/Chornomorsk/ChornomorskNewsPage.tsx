...
import { useGetChornomorskNewsByIdQuery } from '@/redux/services/regionsNews/Odesa/chornomorsk';
moment.locale('uk');

const ChornomorskNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetChornomorskNewsByIdQuery(params.id || '');
...
export default ChornomorskNewsPage;
