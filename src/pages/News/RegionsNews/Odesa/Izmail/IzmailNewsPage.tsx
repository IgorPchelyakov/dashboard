...
import { useGetIzmailNewsByIdQuery } from '@/redux/services/regionsNews/Odesa/izmail';
moment.locale('uk');

const IzmailNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetIzmailNewsByIdQuery(params.id || '');
