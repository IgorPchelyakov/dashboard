...
import { useGetKiliyaNewsByIdQuery } from '@/redux/services/regionsNews/Odesa/kiliya';
moment.locale('uk');

const KiliyaNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetKiliyaNewsByIdQuery(params.id || '');
