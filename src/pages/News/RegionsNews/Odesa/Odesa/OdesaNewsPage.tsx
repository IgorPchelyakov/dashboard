...
import { useGetOdesaNewsByIdQuery } from '@/redux/services/regionsNews/Odesa/odesa';
moment.locale('uk');

const OdesaNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetOdesaNewsByIdQuery(params.id || '');
