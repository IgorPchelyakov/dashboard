...
import { useGetPodilskNewsByIdQuery } from '@/redux/services/regionsNews/Odesa/podilsk';
moment.locale('uk');

const PodilskNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetPodilskNewsByIdQuery(params.id || '');
...
export default PodilskNewsPage;
