...
import { useGetYouzhneNewsByIdQuery } from '@/redux/services/regionsNews/Odesa/youzhne';
moment.locale('uk');

const YouzhneNewsPage: FC = () => {
  const navigate = useNavigate();
  const [error, setError] = useState('');
  const params = useParams<{ id: string }>();

  const { data, isLoading } = useGetYouzhneNewsByIdQuery(params.id || '');
...
export default YouzhneNewsPage;
